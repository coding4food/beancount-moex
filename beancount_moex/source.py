...
def _get_quote(ticker: str, date=None):
    ...
    if len(ticker_parts) == 4:
        ...
        url = url_template.format(engine=engine, market=market, board=board, symbol=symbol, date=date.date() if date else None)
    elif len(ticker_parts) == 3:
        engine, market, symbol = ticker_parts
        url_template = DATE_URL if date is not None else LATEST_URL
        url = url_template.format(engine=engine, market=market, symbol=symbol, date=date.date() if date else None)
    else:
